...
{
public:
    Executor(Grid & planner_grid, Grid & real_grid): planner_grid(planner_grid), rows(planner_grid.rows), cols(planner_grid.cols), real_grid(real_grid){}

    void send_plan(const vector<State>& next);
    void set_logger(Logger* logger){this->logger = logger;}
    vector<State> get_agent_locations(int timestep);

protected:
    Logger* logger = nullptr;
    const Grid planner_grid; // Assume transform between 2 grids is identity for now
    int rows;
    int cols;


    Grid real_grid; // The real grid the robot's act within
    ...
    FreeState place_on_map(State& place) {
        return FreeState{
            location_to_x(place.location),
            location_to_y(place.location),
            orientation_to_theta(place.orientation),
            };
    };
    State real_to_planner (FreeState&) {
        // Invert? Should I just convert on only 1 side?
    };
