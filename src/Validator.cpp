...
bool ValidatorRotate::is_valid(vector<State>& prev, vector<State> & next){
    if (prev.size() != next.size()) {
        errors.push_back(make_tuple(7,-1,-1,prev[0].timestep+1));
        return false;
    }
    ...
    for (int i = 0; i < prev.size(); i ++) {
        ...
        if (abs(prev[i].orientation - next[i].orientation) == 2){
          ...
          return false;
            ...
            if (abs(next[i].location / cols - prev[i].location/cols) + abs(next[i].location % cols - prev[i].location %cols) > 1  ){
                ...
                errors.push_back(make_tuple(3,i,-1,next[i].timestep));
                return false;
            }
        }
        ...
        if (grid.map[next[i].location] == 1) {
            ...
            errors.push_back(make_tuple(4,i,-1,next[i].timestep));
            return false;
        }
        ...
        if (occupied.find(next[i].location) != occupied.end()) {
          ...
          errors.push_back(make_tuple(5,i,occupied[next[i].location],next[i].timestep));
          return false;
        }
        ...
        if (occupied.find(edge_idx) != occupied.end()) {
          ...
          errors.push_back(make_tuple(6,i,occupied[edge_idx],next[i].timestep));
          return false;
        }
